...
@section('page_title', 'Perbarui Galeri Kegiatan')
    ...
    <div class="admin-card overflow-hidden reveal-on-scroll">
        <div class="px-6 py-4 border-b border-platinum bg-soft-grey/30">
            <h3 class="text-[11px] font-black text-midnight-blue uppercase tracking-widest flex items-center gap-2">
                <i data-lucide="edit-3" class="w-4 h-4 text-gold-dignity"></i> Formulir Edit Galeri
            </h3>
        </div>

        <form action="{{ route('admin.galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-8 space-y-2">
                    ...
                    <input type="text" name="judul" required value="{{ old('judul', $galeri->judul) }}" placeholder="Masukkan judul kegiatan..." 
                ...
                <div class="md:col-span-4 space-y-2">
                    ...
                    <input type="date" name="tanggal" required value="{{ old('tanggal', $galeri->tanggal->format('Y-m-d')) }}" 
                ...
                <div class="md:col-span-6 space-y-2">
                    ...
                    <div class="relative">
                        <select name="kategori" required class="w-full px-4 py-3 bg-soft-grey border border-platinum rounded text-[13px] font-bold text-midnight-blue focus:ring-1 focus:ring-gold-dignity focus:border-gold-dignity outline-none transition-all appearance-none">
                            ...
                            <option value="Pembinaan" {{ old('kategori', $galeri->kategori) == 'Pembinaan' ? 'selected' : '' }}>Pembinaan</option>
                            <option value="Pelatihan" {{ old('kategori', $galeri->kategori) == 'Pelatihan' ? 'selected' : '' }}>Pelatihan</option>
                ...
                <div class="md:col-span-6 space-y-2">
                    ...
                    <input type="text" name="lokasi" value="{{ old('lokasi', $galeri->lokasi) }}" placeholder="Contoh: Aula Lazismu" 
                        class="w-full px-4 py-3 bg-soft-grey border border-platinum rounded text-[13px] font-bold text-midnight-blue focus:ring-1 focus:ring-gold-dignity focus:border-gold-dignity outline-none transition-all placeholder-slate-300">
                </div>

                <!-- Gambar Upload dengan Preview -->
                <div class="md:col-span-12 space-y-2" x-data="{ photoName: null, photoPreview: '{{ $galeri->gambar_url }}' }">
                    ...
                    <div class="relative w-full h-64 bg-soft-grey rounded border-2 border-dashed border-platinum flex flex-col items-center justify-center cursor-pointer hover:border-gold-dignity transition-colors group" @click="$refs.photo.click()">
                        ...
                        <div x-show="!photoPreview" class="flex flex-col items-center text-slate-400 group-hover:text-gold-dignity transition-colors">
                            ...
                            <span class="text-[9px] mt-1 font-medium text-slate-300">(Biarkan kosong jika tidak berubah)</span>
                        </div>
                ...
                <div class="md:col-span-12 space-y-2">
                    ...
                    <textarea name="deskripsi" rows="5" placeholder="Tuliskan deskripsi singkat mengenai kegiatan ini..." 
                        class="w-full px-4 py-3 bg-soft-grey border border-platinum rounded text-[13px] font-medium text-slate-600 focus:ring-1 focus:ring-gold-dignity focus:border-gold-dignity outline-none transition-all placeholder-slate-300 leading-relaxed">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                </div>
                ...
                <div class="md:col-span-12 space-y-2">
                    ...
                    <div class="flex gap-6">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="published" {{ old('status', $galeri->status) == 'published' ? 'checked' : '' }} class="w-4 h-4 text-midnight-blue focus:ring-gold-dignity border-platinum cursor-pointer">
                            <span class="text-[12px] font-bold text-slate-600 group-hover:text-midnight-blue transition-colors">Publikasikan (Live)</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="draft" {{ old('status', $galeri->status) == 'draft' ? 'checked' : '' }} class="w-4 h-4 text-midnight-blue focus:ring-gold-dignity border-platinum cursor-pointer">

                    ...
                    <tr class="hover:bg-soft-grey/30 transition-all group" :class="selected.includes({{ $item->id }}) ? 'bg-soft-grey' : ''">
                        ...
                        <td class="px-6 py-5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.integrasi.status', $item->id) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="status" value="approved">
                                    <button class="w-8 h-8 flex items-center justify-center bg-white border border-platinum text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm" title="Setujui">
                                ...
                                <form action="{{ route('admin.integrasi.status', $item->id) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="status" value="rejected">
                                    <button class="w-8 h-8 flex items-center justify-center bg-white border border-platinum text-red-600 rounded-xl hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm" title="Tolak">
                                        <i data-lucide="x" class="w-3.5 h-3.5"></i>
                                    </button>
                                </form>
                                <form action="{{ route('admin.integrasi.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="w-8 h-8 flex items-center justify-center bg-white border border-platinum text-slate-400 rounded-xl hover:bg-red-500 hover:text-white hover:border-red-500 transition-all shadow-sm" title="Hapus">
            ...
            <div class="p-4 space-y-4" :class="selected.includes({{ $item->id }}) ? 'bg-soft-grey' : ''">
                ...
                <div class="flex gap-2">
                    ...
                    <form action="{{ route('admin.integrasi.status', $item->id) }}" method="POST" class="flex-1">
                        @csrf
                        <input type="hidden" name="status" value="approved">
                        <button class="w-full py-2.5 bg-midnight-blue text-white rounded text-[10px] font-black uppercase tracking-widest">Setujui</button>
                    </form>
                    <form action="{{ route('admin.integrasi.status', $item->id) }}" method="POST" class="flex-1">
                        @csrf
                        <input type="hidden" name="status" value="rejected">
                        <button class="w-full py-2.5 border border-platinum text-midnight-blue rounded text-[10px] font-black uppercase tracking-widest">Tolak</button>
                    </form>
                    <form action="{{ route('admin.integrasi.destroy', $item->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button class="py-2.5 px-3 bg-red-50 text-red-600 rounded border border-red-100 hover:bg-red-600 hover:text-white transition-all">

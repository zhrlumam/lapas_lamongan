    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header class="bg-white border-b-4 border-dignity h-20 flex items-center justify-between px-6 z-20 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-imipas uppercase tracking-wider text-sm hidden md:block">Pusat Layanan Informasi</h1>
            </div>
                    ...
                    <table id="tabelInfo" class="w-full">
                        ...
                        <tbody class="text-[11px]">
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="p-4">
                                    <p class="text-imipas uppercase leading-tight italic"><?= htmlspecialchars($row['judul_info']) ?></p>
                                    <p class="text-[10px] text-slate-400 mt-1 line-clamp-1"><?= htmlspecialchars($row['deskripsi_singkat']) ?></p>
                                </td>
                                <td class="p-4 hidden md:table-cell">
                                    <?php if($row['link_tujuan']): ?>
                                        <span class="text-blue-500 italic"><i class="fa-solid fa-link mr-1"></i>Tersedia</span>
                                    <?php else: ?>
                                ...
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button onclick='editInfo(<?= json_encode($row) ?>)' class="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-imipas hover:text-dignity transition-all border border-blue-100">
                                            <i class="fa-solid fa-edit text-[10px]"></i>
                                        </button>
                                        <button onclick="confirmDelete(<?= $row['id_info'] ?>)" class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all border border-red-100">
...
<div id="modalInfo" class="fixed inset-0 bg-[#07213D]/70 backdrop-blur-sm hidden flex items-center justify-center z-[100] p-4">
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl relative overflow-hidden flex flex-col">
        ...
        <form method="POST" class="p-6 space-y-4">
            ...
            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Judul Informasi</label>
                <input type="text" name="judul_info" id="judul_info" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-imipas focus:ring-2 focus:ring-imipas/20 transition-all" required>
            </div>
            
            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Deskripsi Singkat</label>
                <textarea name="deskripsi_singkat" id="deskripsi_singkat" rows="4" class="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 outline-none text-[12px] leading-relaxed" required></textarea>
            </div>

            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Link Tujuan (Opsional)</label>
                <input type="text" name="link_tujuan" id="link_tujuan" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-xs text-blue-500" placeholder="https://...">
            </div>

            <button type="submit" name="simpan_informasi" class="w-full py-4 bg-imipas text-dignity rounded-xl text-[10px] uppercase tracking-[0.2em] shadow-lg hover:brightness-110 transition-all border-b-4 border-slate-900">

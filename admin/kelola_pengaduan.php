...
<head>
    ...
    <style>
        ...
        .dataTables_wrapper .dataTables_filter input {
            padding: 8px 16px; border-radius: 12px; border: 1px solid #e2e8f0; outline: none; margin-bottom: 20px; font-size: 11px;
        }
        table.dataTable thead th { 
            background: #07213D !important; color: #EEBF63 !important; font-size: 10px; 
            text-transform: uppercase; letter-spacing: 0.05em; padding: 15px !important; 
            border: none !important; font-weight: 700;
        }
        .dataTables_wrapper .dataTables_paginate .paginate_button.current { 
            background: #07213D !important; color: #EEBF63 !important; border: none !important; border-radius: 10px; font-size: 11px;
        }
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header class="bg-white border-b-4 border-dignity h-20 flex items-center justify-between px-6 z-20 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-imipas font-bold uppercase tracking-wider text-sm hidden md:block">Manajemen Pengaduan</h1>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="w-10 h-10 rounded-full border-2 border-dignity flex items-center justify-center bg-slate-100 shadow-inner">
        ...
        <main class="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group hover:border-imipas transition-all">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1 relative z-10">Masuk</p>
                    <h3 class="text-3xl font-bold text-imipas relative z-10"><?= $stat_masuk ?></h3>
                    <i class="fa-solid fa-envelope absolute -right-2 -bottom-2 text-slate-50 text-5xl group-hover:text-red-50 transition-colors"></i>
                </div>
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group hover:border-amber-400 transition-all">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1 relative z-10">Proses</p>
                    <h3 class="text-3xl font-bold text-amber-500 relative z-10"><?= $stat_proses ?></h3>
                    <i class="fa-solid fa-spinner absolute -right-2 -bottom-2 text-slate-50 text-5xl group-hover:text-amber-50 transition-colors"></i>
                </div>
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group hover:border-emerald-400 transition-all">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1 relative z-10">Selesai</p>
                    <h3 class="text-3xl font-bold text-emerald-500 relative z-10"><?= $stat_selesai ?></h3>
                    <i class="fa-solid fa-check-double absolute -right-2 -bottom-2 text-slate-50 text-5xl group-hover:text-emerald-50 transition-colors"></i>
                </div>
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group hover:border-slate-400 transition-all">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1 relative z-10">Ditolak</p>
                    <h3 class="text-3xl font-bold text-slate-300 relative z-10"><?= $stat_ditolak ?></h3>
                    <i class="fa-solid fa-ban absolute -right-2 -bottom-2 text-slate-50 text-5xl group-hover:text-slate-100 transition-colors"></i>
                </div>
            </div>

            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-6 overflow-hidden transition-all">
                    ...
                    <table id="tabelAduan" class="w-full">
                        ...
                        <tbody class="text-[11px]">
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/80 transition-all group">
                                ...
                                <td class="p-4">
                                    <p class="text-imipas font-bold uppercase whitespace-nowrap"><?= tgl_indo($row['created_at']) ?></p>
                                    <p class="text-[10px] text-slate-400 font-semibold"><i class="fa-regular fa-clock mr-1"></i><?= date('H:i', strtotime($row['created_at'])) ?> WIB</p>
                                </td>
                                <td class="p-4 min-w-[200px]">
                                    <p class="text-slate-800 font-bold uppercase mb-1 line-clamp-1 group-hover:text-imipas"><?= htmlspecialchars($row['judul_pengaduan']) ?></p>
                                    <span class="bg-slate-100 text-slate-500 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider"><?= htmlspecialchars($row['nama_kategori'] ?? 'Umum') ?></span>
                                </td>
                                <td class="p-4 whitespace-nowrap">
                                    <p class="text-imipas font-bold uppercase"><?= htmlspecialchars($row['nama_pelapor']) ?></p>
                                    <p class="text-emerald-600 font-semibold italic"><i class="fa-brands fa-whatsapp mr-1"></i><?= htmlspecialchars($row['kontak_pelapor']) ?></p>
                                </td>
                                <td class="p-4 whitespace-nowrap">
                                    <span class="px-3 py-1 rounded-lg text-[9px] font-bold uppercase border <?= $badge ?>">
                                        <?= htmlspecialchars($s) ?>
                                    </span>
                                </td>
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="admin_balas.php?id=<?= $row['id'] ?>" class="w-8 h-8 flex items-center justify-center bg-imipas text-dignity rounded-lg hover:scale-110 transition-transform shadow-sm border border-slate-700" title="Proses / Balas">
                                            <i class="fa-solid fa-reply text-[10px]"></i>
                                        </a>
                                        <button onclick="confirmDelete(<?= $row['id'] ?>)" class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg border border-red-100 hover:bg-red-500 hover:text-white transition-all shadow-sm" title="Hapus">

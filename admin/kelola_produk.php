...
if (isset($_POST['simpan_produk'])) {
    ...
    if (!empty($_FILES['gambar']['name'])) {
        ...
        if (in_array($file_ext, $allowed) && $file_size <= 2000000) {
        ...
        <main class="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-6 overflow-hidden">
                <div class="overflow-x-auto custom-scrollbar">
                    <table class="w-full">
                        <thead>
                            <tr class="bg-imipas text-dignity text-[10px] uppercase tracking-widest">
                                <th class="p-4 w-16 text-center rounded-l-xl">No</th>
                                <th class="p-4 text-left">Informasi Produk</th>
                        ...
                        <tbody class="text-[11px]">
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="p-4">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-lg bg-slate-100 overflow-hidden border border-slate-200 shadow-sm flex-shrink-0">
                                            <?php if($r['gambar']): ?>
                                                <img src="../uploads/<?= htmlspecialchars($r['gambar']) ?>" class="w-full h-full object-cover">
                                            <?php else: ?>
                                                <div class="w-full h-full flex items-center justify-center text-slate-300 text-[8px]">No Img</div>
                                            <?php endif; ?>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-imipas uppercase leading-tight line-clamp-1 italic"><?= htmlspecialchars($r['nama_produk']) ?></p>
                                            <p class="text-[9px] text-slate-400 mt-1 line-clamp-1"><?= htmlspecialchars(strip_tags($r['deskripsi'])) ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-4">
                                    <span class="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] uppercase tracking-tighter border border-blue-100">
                                        <?= htmlspecialchars($r['kategori']) ?>
                                    </span>
                                </td>
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button onclick='editProduk(<?= json_encode($r) ?>)' class="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-imipas hover:text-dignity transition-all border border-blue-100">
                                            <i class="fa-solid fa-edit text-[10px]"></i>
                                        </button>
                                        <button onclick="konfirmasiHapus(<?= $r['id_produk'] ?>)" class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all border border-red-100">
                ...
                <div class="mt-6 flex items-center justify-between">
                    <p class="text-[10px] text-slate-400 uppercase tracking-widest">Hal <?= $halaman ?> dari <?= $total_halaman ?></p>
                    <div class="flex gap-2">
                        <?php if($halaman > 1): ?>
                            <a href="?halaman=<?= $halaman - 1 ?>" class="px-4 py-2 bg-slate-100 text-imipas rounded-xl text-[10px] uppercase hover:bg-imipas hover:text-dignity transition-all">Prev</a>
                        <?php endif; ?>
                        <?php if($halaman < $total_halaman): ?>
                            <a href="?halaman=<?= $halaman + 1 ?>" class="px-4 py-2 bg-imipas text-dignity rounded-xl text-[10px] uppercase hover:brightness-125 transition-all">Next</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<div id="modal" class="fixed inset-0 bg-[#07213D]/70 backdrop-blur-sm hidden flex items-center justify-center z-[100] p-4">
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl relative overflow-hidden flex flex-col">
        ...
        <form method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_produk" id="in_nama_produk" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-imipas text-xs focus:ring-2 focus:ring-imipas/20 transition-all" required>
            </div>

            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Kategori</label>
                <input type="text" name="kategori" id="in_kategori" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-imipas text-xs" required>
            </div>

            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Deskripsi Produk</label>
                <textarea name="deskripsi" id="in_deskripsi" rows="3" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-[11px] leading-relaxed" required></textarea>
            </div>

            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Foto Produk (Maks 2MB)</label>
                <input type="file" name="gambar" accept="image/*" class="w-full text-[10px] text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-imipas file:text-dignity border border-slate-100 rounded-xl bg-slate-50">
            </div>

            <button type="submit" name="simpan_produk" class="w-full py-4 bg-imipas text-dignity rounded-xl text-[10px] uppercase tracking-[0.2em] shadow-lg hover:brightness-110 active:scale-[0.98] transition-all border-b-4 border-slate-900">

...
// Pesan Error Role
$error_role = $_SESSION['error_role'] ?? "";
...
try {
    ...
    // 5. Statistik Pengunjung (Unique IP)
    $today_date = date('Y-m-d');
    $this_month = date('Y-m') . '%';
    
    $stmt_today = $pdo->prepare("SELECT COUNT(*) FROM visitor_logs WHERE visited_at = ?");
    $stmt_today->execute([$today_date]);
    $stats_today = $stmt_today->fetchColumn() ?: 0;

    $stmt_month = $pdo->prepare("SELECT COUNT(DISTINCT ip_address) FROM visitor_logs WHERE visited_at LIKE ?");
    $stmt_month->execute([$this_month]);
    $stats_month = $stmt_month->fetchColumn() ?: 0;

    $stats_total = $pdo->query("SELECT COUNT(DISTINCT ip_address) FROM visitor_logs")->fetchColumn() ?: 0;

    // 6. Data untuk Grafik (7 Hari Terakhir)
    $chart_data = [];
    $stmt_chart = $pdo->prepare("SELECT COUNT(*) FROM visitor_logs WHERE visited_at = ?");
    for ($i = 6; $i >= 0; $i--) {
        $date = date('Y-m-d', strtotime("-$i days"));
        $label = date('d M', strtotime($date));
        $stmt_chart->execute([$date]);
        $count = $stmt_chart->fetchColumn() ?: 0;
        $chart_data[] = ['label' => $label, 'count' => $count];
    }
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm">
                                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <i class="fa-solid fa-chart-line text-emerald-500"></i> Trafik Pengunjung
                                </h4>
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                        <p class="text-[9px] font-bold text-slate-400 uppercase mb-1">Hari Ini</p>
                                        <p class="text-xl font-bold text-imipas"><?= number_format($stats_today) ?></p>
                                    </div>
                                    <div class="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                                        <p class="text-[9px] font-bold text-emerald-600 uppercase mb-1">Bulan Ini</p>
                                        <p class="text-xl font-bold text-imipas"><?= number_format($stats_month) ?></p>
                                    </div>
                                </div>
                                <div class="mt-3 bg-imipas/5 p-4 rounded-2xl border border-imipas/10 flex justify-between items-center">
                                    <p class="text-[9px] font-bold text-slate-500 uppercase">Total Pengunjung Unik</p>
                                    <p class="text-sm font-bold text-imipas"><?= number_format($stats_total) ?> Orang</p>
                                </div>
                                <div class="mt-6 h-32">

...
if (isset($_POST['simpan_berita'])) {
    ...
    try {
        if (!empty($_FILES['gambar']['name'])) {
            ...
            $gambar = 'news-' . time() . '-' . rand(1000,9999) . '.' . $ext;
...
<head>
    ...
    <style>
        ...
        .dataTables_wrapper .dataTables_filter input {
            padding: 8px 16px; border-radius: 12px; border: 1px solid #e2e8f0; outline: none; margin-bottom: 20px; font-size: 11px; width: 200px;
        }
        ...
        <main class="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
            <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                <div class="flex gap-4">
                    <div class="bg-white px-6 py-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
                    ...
                    <div class="bg-white px-6 py-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
                        <div class="text-amber-500 bg-amber-50 w-8 h-8 rounded-lg flex items-center justify-center text-sm">
                ...
                <button onclick="openModal()" class="bg-imipas text-dignity px-6 py-3 rounded-xl shadow-lg hover:brightness-110 transition-all flex items-center gap-2 border-b-4 border-slate-900 group">
                    ...
                    <table id="tabelBerita" class="w-full">
                        ...
                        <tbody class="text-[11px]">
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="p-4">
                                    <div class="flex items-center gap-4">
                                        <div class="w-16 h-12 rounded-lg overflow-hidden shadow-sm border border-slate-200 flex-shrink-0">
                                            <?php if($row['gambar']): ?>
                                                <img src="../uploads/<?= $row['gambar'] ?>" class="w-full h-full object-cover">
                                            <?php else: ?>
                                                <div class="w-full h-full bg-slate-100 flex items-center justify-center text-slate-300 text-[8px]">No Img</div>
                                            <?php endif; ?>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-imipas uppercase leading-tight line-clamp-1 italic"><?= htmlspecialchars($row['judul']) ?></p>
                                            <p class="text-[9px] text-slate-400 mt-1 md:hidden italic"><i class="fa-regular fa-clock mr-1"></i><?= date('d/m/Y', strtotime($row['tanggal'])) ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-4 whitespace-nowrap hidden md:table-cell">
                                    <span class="text-slate-500 uppercase text-[10px]">
                                        <i class="fa-regular fa-calendar-alt mr-1"></i><?= date('d M Y', strtotime($row['tanggal'])) ?>
                                    </span>
                                </td>
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button onclick='editBerita(<?= json_encode($row) ?>)' class="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-imipas hover:text-dignity transition-all shadow-sm border border-blue-100">
                                            <i class="fa-solid fa-edit text-[10px]"></i>
                                        </button>
                                        <button onclick="confirmDelete(<?= $row['id_berita'] ?>)" class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all shadow-sm border border-red-100">
...
<div id="modalBerita" class="fixed inset-0 bg-[#07213D]/70 backdrop-blur-sm hidden flex items-center justify-center z-[100] p-4">
    <div class="bg-white w-full max-w-xl rounded-3xl shadow-2xl relative overflow-hidden flex flex-col">
        ...
        <form method="POST" enctype="multipart/form-data" class="p-6 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
            ...
            <div>
                ...
                <input type="text" name="judul" id="judul" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-imipas focus:ring-2 focus:ring-imipas/20 transition-all" placeholder="Input Judul..." required>
            </div>
            
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Tanggal</label>
                    <input type="date" name="tanggal" id="tanggal_input" class="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none text-xs" required>
                </div>
                <div>
                    ...
                    <label for="imgInput" class="w-full p-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-100 transition-all">
            ...
            <div id="previewContainer" class="hidden">
                <img id="imgPreview" src="#" class="w-full h-32 object-cover rounded-xl border border-slate-100 shadow-sm">
            </div>

            <div>
                <label class="text-[9px] text-slate-400 uppercase tracking-widest mb-1 block">Isi Berita</label>
                <textarea name="isi" id="isi" rows="6" class="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 outline-none text-[13px] leading-relaxed" placeholder="Tuliskan berita lengkap..." required></textarea>
            </div>

            <button type="submit" name="simpan_berita" class="w-full py-3.5 bg-imipas text-dignity rounded-xl text-[10px] uppercase tracking-[0.2em] shadow-lg hover:brightness-110 active:scale-[0.98] transition-all border-b-4 border-slate-900">
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({ 
            ...
            if (result.isConfirmed) window.location.href = 'kelola_berita.php?hapus=' + id; 
        });
    }
